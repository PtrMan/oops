// OOPS.cpp : Definiert den Einstiegspunkt für die Konsolenanwendung.
//
...
{
	if (Sp >= stacksize) {
		printf("\nStack overflow"); {halt = TRUE; return; }
	}
...
long z(long a) {
	if (a <= 0) { /*then a in s[]; else a in q[]*/
		return tape[task][-a];
	}
	return  q[a].Q;
}
...
void pushD(long x) {
	if (Dp == maxdp || x < minint || maxint< x) { halt = TRUE; return; }
		set(aDp, Dp + 1);         set(aDp + Dp, x);
}
long popD() {
	long    tmp = Dp;     if (tmp == 0) { halt = TRUE; return 0; }
		set(aDp, tmp - 1);     return  c(aDp + tmp);
}
long topD() { long x = Dp;     if (x == 0) { halt = TRUE; return 0; } return  c(aDp + x); }

long C(long a) { return  tape[task][-acp - a - cp]; }
void setC(long a, long    val)   /*set local vars in a top frame*/
{
	if (maxint< val || val   < minint) { halt = TRUE; return; }   /*out of bounds*/
		set(acp + cp + a, val);
...
void divide() { long x = pop();  if (x == 0) { halt = TRUE; return; } push(pop() / x); }
void remnant() { long x = pop();  if (x == 0) { halt = TRUE; return; } push(pop() % x); }
...
void inb() {
	long n = pop();  n = base + n;/*top to base+n*/
...
void delD() { long x = Dp;     if (x == 0) { halt = TRUE; return; }    set(aDp, x - 1); }
...
void exec() { long n = pop(); if (n<1 || n> nQs) { halt = TRUE; return; }(*Q[n].fn)(); }
...
void topf() { /*push the top selfdefined fn*/
	push(fnp);
}
...
void oldf() {
	long a = pop();  if (a<0 || a>oldp) { halt = TRUE; return; } /*bad old*/
		pushblock(0, 0); setC(1, old[a].start);
} /*last in last out ok*/
...
void decSQ() {
	...
	y = psum();    z = pmax();
	if (x == 1 && y <= z + 1) { halt = TRUE; return; }    /*leave at least 2 SQs*/
		if (x == z)    setmax(get2ndmax());  /*change of max*/
	setp(i, x - 1);   setsum(y - 1);
}                 /*normalizer -1*/
...
void setpat() {
	...
	if (i<0 || i > patp) { halt = TRUE; return; } /*no such search pattern exists*/
		set(acurp, i);
}/*next SQ-search defined via new probabilities!*/

void pupat() {
	long    i = apatp;   /*push search pattern*/
	if (i>maxpat) { halt = TRUE; return; }    i++;    set(apatp, i);  /*not too many?*/
	cpabn(poff + 1, acurp + 1 + i *(2 + nSQs), 2 + nSQs,
		acurp + 1, aendpats);
}

void popat() {
	long i = apatp;  if (i == 0) { halt = TRUE; return; }
		set(apatp, i - 1); push(i);
}                /*pop search pattern*/
...
long gettop(long    tower) {
	return       c(hanoff);
}

long getdisk(long    tower, long    ptr) {
	return c(hanoff + ptr);
}

void settop(long    tower, long    val) {
	set(hanoff, val);
}

void setdisk(long    tower, long ptr, long val) {
	set(hanoff + ptr, val);
}

void inittaskhanoi(long    n) {
	long i;
...
void movdisk(long a, long b) {
	long    topa = gettop(a), topb;
	if (topa == 0) { halt = TRUE; return; }            topb = gettop(b);
	if (topb > 0 && getdisk(b, topb)<getdisk(a, topa)) { halt = TRUE; return; }
		topb++;         settop(b, topb); setdisk(b, topb, getdisk(a, topa));
	setdisk(a, topa, 0);             settop(a, topa - 1);
}
void printhanoi() {
	long tower, tmp, i; printf("\n\nENV:  ");
...
void mvdsk() {
	...
	if (a<1 || a>3 || b<1 || b>3 || a == b) { halt = TRUE; return; } movdisk(a, b);
}
...
void initsimple() {
	nQs = 0;        enter(onetoD)   enter(twotoD)
...
void probsymm(long n) {
	task = n;         initmaxent();
	boost(c1)       boost(c2)       boost(dec)      boost(by2)
		boost(oldSQ)
}
void probhanoi(long n) {
	task = n;         initmaxent();
...
void printblock(long a) {
	long k; printf("%d:", a - acp);
	printf("[ip %d ", c(++a));       printf("dp %d ", c(++a));
	printf("out %d", c(++a));        printf("]  ");
}
void printpat(long n) {
